/*
  ...
  - You are about to alter the column `waktu_transaksi` on the `transaction` table. The data in that column could be lost. The data in that column will be cast from `Timestamp(0)` to `Timestamp`.
...
-- AlterTable
ALTER TABLE `orders` MODIFY `token_transaction` VARCHAR(255) NULL,
    MODIFY `created_at` TIMESTAMP NOT NULL,
    MODIFY `updated_at` TIMESTAMP NOT NULL;
...
-- AlterTable
ALTER TABLE `transaction` MODIFY `waktu_transaksi` TIMESTAMP NOT NULL;
